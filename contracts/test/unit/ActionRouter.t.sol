...
contract ActionRouterTest is Test {
    ...
    function setUp() public {
        ...
        pt = new PrincipalToken("fPT", "fPT", address(sy), market.expiry(), address(market), 18);
    ...
    function test_deadline_revertsPastDeadline() public {
        vm.startPrank(alice);
        underlying.approve(address(router), 1_000e18);
        vm.warp(block.timestamp + 1);
        uint256 oldDeadline = block.timestamp - 1;
        vm.expectRevert(ActionRouter.DeadlineExpired.selector);
        router.depositAndSplit(market, address(underlying), 1_000e18, 0, alice, oldDeadline);
        vm.stopPrank();
    }
    ...
    function test_deadline_zeroSkipsCheck() public {
        ...
        router.depositAndSplit(market, address(underlying), 1_000e18, 0, alice, 0);
        vm.stopPrank();
    }
    ...
    function test_depositAndSplit_underlyingToPtYt() public {
        ...
        (uint256 ptOut, uint256 ytOut) = router.depositAndSplit(
            market, address(underlying), 1_000e18, 0, alice, block.timestamp + 60
        );
        vm.stopPrank();

        // 1:1 deposit (mock SY) and 1:1 split → 1000 PT and 1000 YT.
        assertEq(ptOut, 1_000e18);
        assertEq(ytOut, 1_000e18);
    ...
    function test_depositAndSplit_slippage() public {
        ...
        router.depositAndSplit(market, address(underlying), 1_000e18, 1_001e18, alice, 0);
        vm.stopPrank();
    }
    ...
    function test_swapExactSyForPt_buysPt() public {
        ...
        vm.startPrank(alice);
        underlying.approve(address(router), 5_000e18);
        router.depositAndSplit(market, address(underlying), 5_000e18, 0, alice, 0);

        // Now alice has 5000 PT + 5000 YT. Use SY (from a fresh deposit) to buy more PT.
        underlying.approve(address(sy), 5_000e18);
        sy.deposit(alice, address(underlying), 5_000e18, 0);

        IERC20(address(sy)).approve(address(router), 5_000e18);
        uint256 ptBefore = pt.balanceOf(alice);
        uint256 ptDesired = 1_000e18;
        uint256 syUsed = router.swapExactSyForPt(market, 5_000e18, ptDesired, alice, 0);
        uint256 ptAfter = pt.balanceOf(alice);
        vm.stopPrank();

        assertEq(ptAfter - ptBefore, ptDesired);
        assertGt(syUsed, 0);
        assertLt(syUsed, 5_000e18);
    ...
    function test_buyYT_userGetsYTAndRefund() public {
        vm.startPrank(alice);
        underlying.approve(address(sy), 10_000e18);
        sy.deposit(alice, address(underlying), 10_000e18, 0);
        // Alice has 10000 SY now.

        IERC20(address(sy)).approve(address(router), 5_000e18);
        uint256 syBefore = IERC20(address(sy)).balanceOf(alice);
        (uint256 ytOut, uint256 syRefund) = router.buyYT(market, 5_000e18, 0, alice, 0);
        uint256 syAfter = IERC20(address(sy)).balanceOf(alice);
        vm.stopPrank();

        // ytOut = 5_000 (split is 1:1). syRefund = SY received from PT sale.
        assertEq(ytOut, 5_000e18);
        assertEq(yt.balanceOf(alice), 5_000e18);

        // Net SY out of alice's pocket = 5_000 - syRefund.
        // syBefore was 10_000 (after deposit), alice paid 5_000 SY in, got syRefund back.
        assertEq(syAfter, syBefore - 5_000e18 + syRefund);
        assertGt(syRefund, 0);
        assertLt(syRefund, 5_000e18); // PT sells at a discount; refund < SY paid
    }
    ...
    function test_buyYT_slippageOnPtSale() public {
        ...
        vm.expectRevert();
        router.buyYT(market, 5_000e18, 5_000e18, alice, 0);
        vm.stopPrank();
    }
    ...
    function test_addLiquidityProportional() public {
        vm.startPrank(alice);
        underlying.approve(address(router), 10_000e18);
        router.depositAndSplit(market, address(underlying), 10_000e18, 0, alice, 0);
        ...
        IERC20(address(sy)).approve(address(router), 5_000e18);
        IERC20(address(pt)).approve(address(router), 5_000e18);
        uint256 lpBefore = market.balanceOf(alice);
        uint256 lpOut = router.addLiquidityProportional(market, 5_000e18, 5_000e18, 0, alice, 0);
    ...
    function test_removeLiquidityProportional() public {
        ...
        vm.prank(admin);
        market.transfer(alice, 10_000e18);

        vm.startPrank(alice);
        market.approve(address(router), 10_000e18);
        (uint256 syOut, uint256 ptOut) = router.removeLiquidityProportional(
            market, 10_000e18, 0, 0, alice, 0
        );
        vm.stopPrank();
    ...
    function test_redeemAfterExpiryAndUnwrap() public {
        ...
        vm.startPrank(alice);
        IERC20(address(pt)).approve(address(router), 1_000e18);
        IERC20(address(yt)).approve(address(router), 1_000e18);
        uint256 underBefore = underlying.balanceOf(alice);
        uint256 amountOut = router.redeemAfterExpiryAndUnwrap(
            market, 1_000e18, 1_000e18, address(underlying), 0, alice, 0
        );
        ...
        assertGt(amountOut, 952e18);
        assertLt(amountOut, 953e18);
    }
    ...
    function test_unwrapSY_sharesToUnderlying() public {
        vm.startPrank(alice);
        underlying.approve(address(sy), 500e18);
        sy.deposit(alice, address(underlying), 500e18, 0);

        IERC20(address(sy)).approve(address(router), 500e18);
        uint256 underBefore = underlying.balanceOf(alice);
        uint256 amountOut = router.unwrapSY(sy, 500e18, address(underlying), 0, alice, 0);
        uint256 underAfter = underlying.balanceOf(alice);
        vm.stopPrank();

        assertEq(underAfter - underBefore, amountOut);
        assertEq(amountOut, 500e18); // mock is 1:1
    }
