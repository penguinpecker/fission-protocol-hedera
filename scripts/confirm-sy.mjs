#!/usr/bin/env node
// Calls factory.confirmSY(sy) for one or more SYs after the 7-day proposeSY
// review window has elapsed. Caller must hold DEFAULT_ADMIN_ROLE on the factory.
//
// Usage:
//   node scripts/confirm-sy.mjs <factory-evm-address> <sy-evm-address> [<sy-evm-address> ...]
...
if (args.length < 2 || !args[0].startsWith("0x")) {
  console.error("Usage: node scripts/confirm-sy.mjs <factory-evm-address> <sy-evm-address> [<sy-evm-address> ...]");
  process.exit(1);
}
...
for (const syEvm of syEvms) {
  ...
  const tx = new ContractExecuteTransaction()
    ...
    .setFunction("confirmSY", new ContractFunctionParameters().addAddress(syEvm));
...
console.log(`\n✓ All confirmed. Next: createMarket / createRewardsMarket.`);
client.close();
